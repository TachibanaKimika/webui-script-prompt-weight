...
def apply_prompt_weight(p, x, xs):
    if xs[0] not in p.prompt and xs[0] not in p.negative_prompt:
        raise RuntimeError(f"Prompt S/R did not find {xs[0]} in prompt or negative prompt.")
    
    p.prompt = p.prompt.replace(xs[0], "(" + xs[0] + ":" + x + ")")
    p.negative_prompt = p.negative_prompt.replace(xs[0], "(" + xs[0] + ":" + x + ")")
...
axis_options = [
    ...
    AxisOption("Prompt Weight", str, apply_prompt_weight, format_value, None),
